class Note {
  constructor(index, note, interval, isScale, x, y, r, key) {
    ...
    // Variables pour stocker les paramètres de fadeIn et fadeOut
    this.fadeInSpeed = 10
    this.fadeOutSpeed = 10
    this.anim = fadeIn(this.fadeInSpeed)
    this.anim2 = fadeOut(this.fadeOutSpeed)
  }
  ...
  update(x, y, r) {
    ...
    this.x = x + 0.8 * r * Math.cos(this.a - HALF_PI)
    this.y = y + 0.8 * r * Math.sin(this.a - HALF_PI)
    textSize(0.8 * this.size) // Taille de texte proportionnelle à la taille de la fenêtre
  }

  getStatus() {
    // zones sensibles / interactions souris
    ...
    if (this.noteIsHovered != this.previous) {
      this.previous = this.noteIsHovered
      this.isEnteringNote = this.noteIsHovered
      if (!this.isEnteringNote)
        this.anim = fadeOut(this.fadeOutSpeed)
    }
  }
  ...
  displayDegre() {
    ...
    if (this.degreIsHovered) {
      ...
      stroke(this.a * 100, 455 + 200 * this.animate(1), 470 + 200 * this.animate(1))
    } else {
      strokeWeight(5)
      stroke(this.a * 100, 455, 470)
    }
    ...
    noFill()
    if (this.isScale) {
      arc(this.xCenter, this.yCenter, 0.7 * 2 * this.r, 0.7 * 2 * this.r, this.a - PI / 12 - HALF_PI, this.a + PI / 12 - HALF_PI)
    }

    // cercle
    if (this.noteIsHovered) {
      strokeWeight(this.r / 5 + this.anim())
      arc(this.xCenter, this.yCenter, 0.8 * 2 * this.r, 0.8 * 2 * this.r, this.a - PI / 12 - HALF_PI, this.a + PI / 12 - HALF_PI)
    } else
      this.anim = fadeOut(this.fadeOutSpeed)
  ...
  displayNote() {
    push()
    if (this.noteIsHovered) {
      strokeWeight(5 * this.animate(0.01))
      stroke(this.a * 100, 455 + 100 * this.animate(0.01), 470 + 100 * this.animate(0.01))
    } else {
      strokeWeight(5)
      stroke(this.a * 100, 455, 470)
    }
    ...
    // cercle exterieur
    arc(this.xCenter, this.yCenter, 0.9 * 2 * this.r, 0.9 * 2 * this.r, this.a - PI / 12 - HALF_PI, this.a + PI / 12 - HALF_PI)
  ...
  pressed() {
    ...
    this.anim = fadeIn(this.fadeInSpeed) // reinit de l animation
  }
